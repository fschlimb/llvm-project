// RUN: mlir-opt \
// RUN:   --pass-pipeline="builtin.module(func.func(mesh-spmdization),canonicalize)" \
// RUN:   %s | FileCheck %s
...
// CHECK-LABEL: func @sharding_triplet_odd
func.func @sharding_triplet_odd(
  // CHECK-SAME: %[[ARG:.*]]: tensor<?xf32>
  %arg0: tensor<3xf32>
// CHECK-SAME: ) -> tensor<3xf32> {
) -> tensor<3xf32> {
  // CHECK: %[[ALL_GATHER:.*]] = mesh.all_gather %[[ARG]] on @mesh_1d mesh_axes = [0] gather_axis = 0 : tensor<?xf32> -> tensor<3xf32>
  %ssharding_annotated = mesh.sharding @mesh_1d split_axes = [[0]] : !mesh.sharding
  %sharding_annotated = mesh.shard %arg0 to %ssharding_annotated  : tensor<3xf32>
  %ssharding_annotated_0 = mesh.sharding @mesh_1d split_axes = [[0]] : !mesh.sharding
  %sharding_annotated_0 = mesh.shard %sharding_annotated to %ssharding_annotated_0  annotate_for_users : tensor<3xf32>
  %ssharding_annotated_1 = mesh.sharding @mesh_1d split_axes = [[]] : !mesh.sharding
  %sharding_annotated_1 = mesh.shard %sharding_annotated_0 to %ssharding_annotated_1  : tensor<3xf32>
  // CHECK: return %[[ALL_GATHER]] : tensor<3xf32>
  return %sharding_annotated_1 : tensor<3xf32>
}
...
// CHECK-LABEL: func @move_split_axis_odd
func.func @move_split_axis_odd(
  // CHECK-SAME: %[[ARG:.*]]: tensor<?x3xi8>
  %arg0: tensor<3x3xi8>
// CHECK-SAME: -> tensor<3x?xi8> {
) -> tensor<3x3xi8> {
  // CHECK: %[[ALL_TO_ALL:.*]] = mesh.all_to_all %[[ARG]] on @mesh_1d
  // CHECK-SAME: mesh_axes = [0] split_axis = 1 concat_axis = 0 : tensor<?x3xi8> -> tensor<3x?xi8>
  %s0 = mesh.sharding @mesh_1d split_axes = [[0]] : !mesh.sharding
  %0 = mesh.shard %arg0 to %s0  : tensor<3x3xi8>
  %s1 = mesh.sharding @mesh_1d split_axes = [[], [0]] : !mesh.sharding
  %1 = mesh.shard %0 to %s1  annotate_for_users : tensor<3x3xi8>
  // CHECK: return %[[ALL_TO_ALL]] : tensor<3x?xi8>
...
// CHECK-LABEL: func @unary_elementwise_odd
func.func @unary_elementwise_odd(
  // CHECK-SAME: %[[ARG:.*]]: tensor<?xi8>
  %arg0: tensor<3xi8>
// CHECK-SAME: -> tensor<?xi8> {
) -> tensor<3xi8> {
  %s0 = mesh.sharding @mesh_1d split_axes = [[0]] : !mesh.sharding
  %0 = mesh.shard %arg0 to %s0  : tensor<3xi8>
  %s1 = mesh.sharding @mesh_1d split_axes = [[0]] : !mesh.sharding
  %1 = mesh.shard %0 to %s1  annotate_for_users : tensor<3xi8>
  // CHECK: %[[RES:.*]] = tosa.abs %[[ARG]] : (tensor<?xi8>) -> tensor<?xi8>
  %2 = tosa.abs %1 : (tensor<3xi8>) -> tensor<3xi8>
  %s3 = mesh.sharding @mesh_1d split_axes = [[0]] : !mesh.sharding
  %3 = mesh.shard %2 to %s3  : tensor<3xi8>
  %s4 = mesh.sharding @mesh_1d split_axes = [[0]] : !mesh.sharding
  %4 = mesh.shard %3 to %s4  annotate_for_users : tensor<3xi8>
  // CHECK: return %[[RES]] : tensor<?xi8>
...
// odd size & full replication -> shard axis -> abs -> shard axis -> full replication
// CHECK-LABEL: func @unary_elementwise_with_resharding_odd
func.func @unary_elementwise_with_resharding_odd(
  // CHECK-SAME: %[[ARG:.*]]: tensor<3xi8>
  %arg0: tensor<3xi8>
// CHECK-SAME: -> tensor<3xi8> {
) -> tensor<3xi8> {
  // CHECK: %[[SLICE:.*]] = mesh.all_slice %[[ARG]] on @mesh_1d mesh_axes = [0] slice_axis = 0
  // CHECK-SAME: tensor<3xi8> -> tensor<?xi8>
  %s0 = mesh.sharding @mesh_1d split_axes = [[]] : !mesh.sharding
  %0 = mesh.shard %arg0 to %s0  : tensor<3xi8>
  %s1 = mesh.sharding @mesh_1d split_axes = [[0]] : !mesh.sharding
  %1 = mesh.shard %0 to %s1  annotate_for_users : tensor<3xi8>
  // CHECK: %[[ABS:.*]] = tosa.abs %[[SLICE]] : (tensor<?xi8>) -> tensor<?xi8>
  %2 = tosa.abs %1 : (tensor<3xi8>) -> tensor<3xi8>
  // CHECK: %[[RES:.*]] = mesh.all_gather %[[ABS]] on @mesh_1d
  // CHECK-SAME: mesh_axes = [0] gather_axis = 0 : tensor<?xi8> -> tensor<3xi8>
  %s3 = mesh.sharding @mesh_1d split_axes = [[0]] : !mesh.sharding
  %3 = mesh.shard %2 to %s3  : tensor<3xi8>
  %s4 = mesh.sharding @mesh_1d split_axes = [[]] : !mesh.sharding
  %4 = mesh.shard %3 to %s4  annotate_for_users : tensor<3xi8>
  // CHECK: return %[[RES]] : tensor<3xi8>
  return %4 : tensor<3xi8>
}
...
// CHECK-LABEL: func @binary_elementwise_odd
func.func @binary_elementwise_odd(
  // CHECK-SAME: %[[ARG0:.*]]: tensor<?xi8>,
  %arg0: tensor<3xi8>,
  // CHECK-SAME: %[[ARG1:.*]]: tensor<?xi8>
  %arg1: tensor<3xi8>
// CHECK-SAME: -> tensor<?xi8> {
) -> tensor<3xi8> {
  %sarg0_sharded = mesh.sharding @mesh_1d split_axes = [[0]] : !mesh.sharding
  %arg0_sharded = mesh.shard %arg0 to %sarg0_sharded  : tensor<3xi8>
  %sop_arg0 = mesh.sharding @mesh_1d split_axes = [[0]] : !mesh.sharding
  %op_arg0 = mesh.shard %arg0_sharded to %sop_arg0  annotate_for_users : tensor<3xi8>
  %sarg1_sharded = mesh.sharding @mesh_1d split_axes = [[0]] : !mesh.sharding
  %arg1_sharded = mesh.shard %arg1 to %sarg1_sharded  : tensor<3xi8>
  %sop_arg1 = mesh.sharding @mesh_1d split_axes = [[0]] : !mesh.sharding
  %op_arg1 = mesh.shard %arg1_sharded to %sop_arg1  annotate_for_users : tensor<3xi8>
  // CHECK: %[[RES:.*]] = tosa.add %[[ARG0]], %[[ARG1]] : (tensor<?xi8>, tensor<?xi8>) -> tensor<?xi8>
  %op_res = tosa.add %op_arg0, %op_arg1 : (tensor<3xi8>, tensor<3xi8>) -> tensor<3xi8>
  %sop_res_sharded = mesh.sharding @mesh_1d split_axes = [[0]] : !mesh.sharding
  %op_res_sharded = mesh.shard %op_res to %sop_res_sharded  : tensor<3xi8>
  %sres = mesh.sharding @mesh_1d split_axes = [[0]] : !mesh.sharding
  %res = mesh.shard %op_res_sharded to %sres  annotate_for_users : tensor<3xi8>
  // CHECK: return %[[RES]] : tensor<?xi8>
  return %res : tensor<3xi8>
}
...
// abs
// reshard
// CHECK-LABEL: func @multiple_chained_ops_odd
func.func @multiple_chained_ops_odd(
  // CHECK-SAME: %[[ARG:.*]]: tensor<3xi8>
  %arg0: tensor<3xi8>
// CHECK-SAME: -> tensor<?xi8> {
) -> tensor<3xi8> {
  ...
  %s1 = mesh.sharding @mesh_1d split_axes = [[0]] : !mesh.sharding
  %1 = mesh.shard %0 to %s1  annotate_for_users : tensor<3xi8>
  // CHECK: %[[ABS1:.*]] = tosa.abs %[[RESHARD1]] : (tensor<?xi8>) -> tensor<?xi8>
  %2 = tosa.abs %1 : (tensor<3xi8>) -> tensor<3xi8>
  // CHECK: %[[RESHARD2:.*]] = mesh.all_gather %[[ABS1]] on @mesh_1d
  // CHECK-SAME: mesh_axes = [0] gather_axis = 0 : tensor<?xi8> -> tensor<3xi8>
  %s3 = mesh.sharding @mesh_1d split_axes = [[0]] : !mesh.sharding
  ...
  %s4 = mesh.sharding @mesh_1d split_axes = [[]] : !mesh.sharding
  %4 = mesh.shard %3 to %s4  annotate_for_users : tensor<3xi8>
  // CHECK: %[[ABS2:.*]] = tosa.abs %[[RESHARD2]] : (tensor<3xi8>) -> tensor<3xi8>
  %5 = tosa.abs %4 : (tensor<3xi8>) -> tensor<3xi8>
  // CHECK: %[[RESHARD3:.*]] = mesh.all_slice %[[ABS2]] on @mesh_1d mesh_axes = [0] slice_axis = 0 : 
  // CHECK-SAME: tensor<3xi8> -> tensor<?xi8>
  %s6 = mesh.sharding @mesh_1d split_axes = [[]] : !mesh.sharding
  %6 = mesh.shard %5 to %s6  : tensor<3xi8>
  %s7 = mesh.sharding @mesh_1d split_axes = [[0]] : !mesh.sharding
  %7 = mesh.shard %6 to %s7  annotate_for_users : tensor<3xi8>
  // CHECK: return %[[RESHARD3]] : tensor<?xi8>
  return %7 : tensor<3xi8>
}
...
// CHECK-LABEL: func @test_shard_update_halo2d_odd
// CHECK-SAME: [[varg0:%[A-Za-z0-9_]+]]: tensor<300x?xi64>) -> tensor<303x?xi64> {
func.func @test_shard_update_halo2d_odd(%arg0: tensor<1200x1201xi64>) -> tensor<1200x1201xi64> {
  %sharding = mesh.sharding @mesh4x4 split_axes = [[0], [1]] : !mesh.sharding
  %sharding_annotated = mesh.shard %arg0 to %sharding : tensor<1200x1201xi64>
  %sharding_0 = mesh.sharding @mesh4x4 split_axes = [[0], [1]] halo_sizes = [1, 2, 3, 4] : !mesh.sharding
  // CHECK-DAG: [[vc1:%.*]] = arith.constant 1 : index
  // CHECK-DAG: [[vc7:%.*]] = arith.constant 7 : index
  // CHECK: [[vdim:%.*]] = tensor.dim [[varg0]], [[vc1]] : tensor<300x?xi64>
  // CHECK: [[v0:%.*]] = arith.addi [[vdim]], [[vc7]] : index
  // CHECK: [[v1:%.*]] = tensor.empty([[v0]]) : tensor<303x?xi64>
  // CHECK: [[vextracted_slice:%.*]] = tensor.extract_slice [[varg0]][0, 0] [300, [[vdim]]] [1, 1] : tensor<300x?xi64> to tensor<300x?xi64>
  // CHECK: [[vinserted_slice:%.*]] = tensor.insert_slice [[vextracted_slice]] into [[v1]][1, 3] [300, [[vdim]]] [1, 1] : tensor<300x?xi64> into tensor<303x?xi64>
  // CHECK: [[v2:%.*]] = mesh.update_halo [[vinserted_slice]] on @mesh4x4 split_axes = {{\[\[}}0], [1]] halo_sizes = [1, 2, 3, 4] : tensor<303x?xi64>
  %sharding_annotated_1 = mesh.shard %sharding_annotated to %sharding_0 : tensor<1200x1201xi64>
  %sharding_annotated_3 = mesh.shard %sharding_annotated_1 to %sharding_0 annotate_for_users : tensor<1200x1201xi64>
  // CHECK: return [[v2]] : tensor<303x?xi64>
  return %sharding_annotated_3 : tensor<1200x1201xi64>
}